    ...
    <header className="header">
      <div className="container header-content">
        ...
        <nav className="header-nav">
          <Dropdown
            trigger={<><LanguageIcon /><span className="dropdown-label">{language.toUpperCase()}</span></>}
            isOpen={langOpen}
            onToggle={() => { closeAll(); setLangOpen(!langOpen); }}
          ...
          <Dropdown
            trigger={<>{getThemeIcon()}<span className="dropdown-label">{t(`theme.${mode}`)}</span></>}
            isOpen={themeOpen}
            onToggle={() => { closeAll(); setThemeOpen(!themeOpen); }}
            onClose={() => setThemeOpen(false)}
          >
            <button 
              className={`dropdown-item ${mode === 'system' ? 'active' : ''}`}
              onClick={() => { setMode('system'); setThemeOpen(false); }}
            >
              <MonitorIcon /> {t('theme.system')}
            </button>
            <button 
              className={`dropdown-item ${mode === 'light' ? 'active' : ''}`}
              onClick={() => { setMode('light'); setThemeOpen(false); }}
            >
              <SunIcon /> {t('theme.light')}
            </button>
            <button 
              className={`dropdown-item ${mode === 'dark' ? 'active' : ''}`}
              onClick={() => { setMode('dark'); setThemeOpen(false); }}
            >
